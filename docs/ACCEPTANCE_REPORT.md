...
- 内容状态：`reviewStatus` 结构化（场景 + 隐私主题），生产入口过滤 draft；**draft 数量 2（s14/s15），reviewed 数量 12 场景 + 4 隐私主题**。

## 未执行项与原因

1. Playwright E2E：BLOCKED（浏览器运行文件下载受限），恢复方式见 TESTING_WITHOUT_PLAYWRIGHT.md §13。
2. 三视口与键盘人工验收：NOT RUN，待人工按 §6–§8 执行。
3. 成年人 SM/BDSM 与情趣内容专业审校：**未完成**。s14/s15 保持 `draft` 不上线；绿黄红体系上线前需熟悉成年人同意教育与 SM/BDSM 社群实践的专业审校者复核。
