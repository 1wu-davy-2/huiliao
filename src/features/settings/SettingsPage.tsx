      ...
      <section className="section" aria-labelledby="privacy-title">
        ...
        <div className="card mt-16">
          ...
          <ul className="stack mt-8">
            <li>首次设置：成年确认、选择的困难。基线判断题的答案不保存。</li>
            <li>进度：完成的情境、提交回应次数、重试次数、五维能力变化；同一场景重复练习只保留最近一次记录。</li>
            <li>收藏的情境，以及你主动点击“保存复盘”的内容。</li>
          </ul>
          <p className="bold mt-16">不保存的内容</p>
          <ul className="stack mt-8">
            <li>消息实验室的原始草稿（诊断后立即丢弃）。</li>
            <li>模拟对话中的自由输入原文。</li>
          </ul>
          <p className="small muted mt-16">
            不需要注册账号，不接入广告、用户画像或跨站跟踪；训练内容（草稿、自由输入、复盘）不上传任何服务器。访问网站时，托管平台（当前为 Vercel）为提供静态文件会处理基础请求元数据（如 IP、User-Agent、请求路径），按其隐私政策处理；项目默认不开启平台分析与速度统计。清除数据后刷新页面，旧数据无法恢复。
          </p>
        </div>
      </section>
